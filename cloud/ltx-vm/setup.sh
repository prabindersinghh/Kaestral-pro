...
cat <<'NOTE'
  ...
  Set your shared secret (must match what you paste into Kaestral):
    sudo sed -i 's/CHANGE_ME_SHARED_SECRET/<your-random-token>/' /etc/systemd/system/kaestral-ltx.service

NOTE

# Install systemd units.
sudo cp systemd/kaestral-ltx.service systemd/kaestral-idle-stop.service systemd/kaestral-idle-stop.timer /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now kaestral-idle-stop.timer     # the credit guard — on immediately
sudo systemctl enable kaestral-ltx                        # starts on next boot; start now once wired:
echo
echo "Idle watchdog is ACTIVE (stops this VM after 15 idle min)."
echo "After you wire the LTX pipeline: sudo systemctl start kaestral-ltx  &&  curl localhost:8000/health"
echo "Then STOP the VM from your PC (Kaestral) or:  sudo shutdown -h now  (or gcloud compute instances stop ...)"
