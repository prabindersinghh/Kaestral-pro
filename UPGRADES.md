# UPGRADES — the honest gap to a real editor

Phase-1 hit **architectural + format + MCP parity** and now shows/exports **real footage**. But it is
NOT yet as complete as the upstream macOS editor as a day-to-day editor. This is the straight list of what still
...
1. 🔴 **Smooth video playback.** The preview decodes real pixels via `<video>` *seeking* — crisp for
   scrubbing and stills, but pressing **Play** stutters because every frame issues a seek. The upstream
   editor plays back smoothly and in real time. **Fix:** WebCodecs `VideoDecoder` with a decoded-frame queue
...
3. 🟠 **Bring in your OWN media (UI).** The app ships real bundled demo media and the MCP `import_media`
   ...
   existing `.kaestral` project and relinking its media is also not in the UI.
...
12. ⛔ **Generative AI** — permanently out of scope (proprietary upstream cloud). Tools return the
    signed-out shape by design; not a gap to close.

## Standing hard gate
- **Parity sign-off vs a real macOS-saved project.** The round-trip golden file is hand-authored from the
...
- Real `.kaestral` load/save (semantic round-trip vs the source-derived golden fixture).
...
- XMEML (Premiere) + FCPXML (Resolve/FCP) + `.kaestral` exporters.
