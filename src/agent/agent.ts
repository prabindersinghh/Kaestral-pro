...
Always call get_timeline first. When the user asks for a title/intro/logo reveal/data-viz/transition, use generate_title (simple text) or generate_motion (complex motion). For a task that matches a skill (viral reel, beat-sync, creative direction, captions, b-roll), read_skill first and follow it. The ONLY way to produce a final video is export_project(mode:'video') — it renders the CURRENT TIMELINE through Kaestral, so the export is exactly what the user watched you build; do that (or tell them to hit Export) as the last step. Keep replies short; let the edits speak.`;
