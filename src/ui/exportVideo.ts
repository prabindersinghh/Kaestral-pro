...
export async function exportVideoFromUI(codec = "H.264", resolution = "1080p", fileName = "palmier-export.mp4"): Promise<ExportResult> {
  ...
  try {
    ...
    if (/tauri|invoke|not.*function|undefined/i.test(msg)) {
      throw new Error("Video export runs in the Palmier desktop app. In the browser preview, export via the MCP export_project tool.");
    }
